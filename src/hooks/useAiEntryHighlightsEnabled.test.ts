...
import { useAiEntryHighlightsEnabled } from './useAiEntryHighlightsEnabled'
...
describe('useAiEntryHighlightsEnabled', () => {
  it('returns null while hydration is in flight', () => {
    ...
    const { result } = renderHook(() => useAiEntryHighlightsEnabled())
  ...
  it('returns true when the store flag is on after hydration', async () => {
    vi.mocked(tauri.getAiSettings).mockResolvedValueOnce({ ...FULL, entryHighlightsEnabled: true })
    const { result } = renderHook(() => useAiEntryHighlightsEnabled())
    await waitFor(() => expect(result.current).toBe(true))
  })

  it('re-renders when the store flag flips after hydration (the bug we are fixing)', async () => {
    vi.mocked(tauri.getAiSettings).mockResolvedValueOnce({ ...FULL, entryHighlightsEnabled: false })
    const { result } = renderHook(() => useAiEntryHighlightsEnabled())
