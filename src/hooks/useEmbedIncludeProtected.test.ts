...
import { useEmbedIncludeProtected } from './useEmbedIncludeProtected'
...
describe('useEmbedIncludeProtected', () => {
  it('defaults to false when the setting is absent', async () => {
    const { result } = renderHook(() => useEmbedIncludeProtected())
  ...
  it("reads true when the stored setting is 'true'", async () => {
    mockedInvoke.mockImplementation(async (cmd, args) => {
      if (cmd === 'get_setting' && (args as { key: string }).key === 'ai_embed_include_protected') {
        return 'true'
      }
      return null
    })
  ...
  it('setIncludeProtected persists and updates the live value', async () => {
    ...
    expect(setCall?.[1]).toEqual({ key: 'ai_embed_include_protected', value: 'true' })
  })
