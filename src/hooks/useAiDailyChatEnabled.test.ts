...
import { useAiDailyChatEnabled } from './useAiDailyChatEnabled'
...
describe('useAiDailyChatEnabled', () => {
  it('returns null while hydration is in flight', () => {
    ...
    const { result } = renderHook(() => useAiDailyChatEnabled())
    expect(result.current).toBeNull()
    resolve(FULL)
  })

  it('returns true when the store flag is on after hydration', async () => {
    vi.mocked(tauri.getAiSettings).mockResolvedValueOnce({ ...FULL, dailyChatEnabled: true })
    const { result } = renderHook(() => useAiDailyChatEnabled())
    await waitFor(() => expect(result.current).toBe(true))
  })

  it('re-renders when the store flag flips after hydration (sidebar live update)', async () => {
    vi.mocked(tauri.getAiSettings).mockResolvedValueOnce({ ...FULL, dailyChatEnabled: false })
    const { result } = renderHook(() => useAiDailyChatEnabled())
